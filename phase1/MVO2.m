function [path,visited_nodes,costValue] = MVO2(map, start, goal,visited_nodes)
...
[sorted_Inflation_rates,sorted_indexes]=sort([agent.Cost]);
    ...
    for newindex=1:nPop
        ...
        sorted(newindex).Cost=sorted_Inflation_rates(newindex);
    end
...
%     end
 ...
 for it=2:nPop
    Back_hole_index = it;
         for j=1:3%for each object
                 r1=rand();
            if r1<normalized_sorted_Inflation_rates(it)
                White_hole_index=RouletteWheelSelection(-normalized_sorted_Inflation_rates);% for maximization problem -sorted_Inflation_rates should be written as sorted_Inflation_rates
                if White_hole_index==-1
                    White_hole_index=1;
                end
                %Eq. (3.1) in the paper
                agent(Back_hole_index).Position(j)=sorted(White_hole_index).universes(j);
            end
            r2=rand();
            if (size(lb,3)==1)
                ...
                if r2<WEP
                    ...
                    r4=rand();
                    if r3<0.5
                        agent(it).Position(j)=Dest.Position(j)+TDR*((ub(j)-lb(j))*r4+lb(j));
            ...
            if (size(lb,3)~=1)
                ...
                universes=agent(it).Position;
                toward = Dest.Position;
            [newPath,Cost] = changePath(universes(:,4),toward(:,4),AllPoints(:,:),CollisionTest,StartNode,GoalNode,nodes,Boundaryinitial,Boundaryfinal,size(DummyX,1),size(DummyY,1),size(DummyZ,1),visited_nodes);
            newPath = removeLoops(newPath);
            agent(it).Position = AllPoints(newPath(:,1),:);
            agent(it).Position(:,4) = newPath(:,1);
            agent(it).Cost = Cost;
        
         end
       if agent(it).Cost < Dest.Cost
           Dest.Cost=agent(it).Cost;
           Dest.Position=agent(it).Position;
       end    
end
